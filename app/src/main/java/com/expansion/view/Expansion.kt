...
import com.robertlevonyan.views.expandable.R
import getCircleBitmap
import getRoundedSquareBitmap
import kotlin.properties.Delegates


class Expansion : FrameLayout {
    var icon: Drawable? = null
        set(value) {
            field = value
            buildView()
        }
    var iconStyle = ExpansionIconStyles.SQUARE
        set(value) {
            field = value
            buildView()
        }
    ...
    private fun initTypedArray(attrs: AttributeSet?) {
        ...
        val ta = context.theme.obtainStyledAttributes(attrs, R.styleable.Expansion, 0, 0)

        icon = ta.getDrawable(R.styleable.Expansion_exp_icon)
        iconStyle = ExpansionIconStyles.getByIndex(ta.getInt(R.styleable.Expansion_exp_iconStyle, ExpansionIconStyles.SQUARE.ordinal))
        animateExpand = ta.getBoolean(R.styleable.Expansion_exp_animateExpand, false)
        backgroundColor = ta.getColor(R.styleable.Expansion_exp_backgroundColor, ContextCompat.getColor(context, R.color.colorDefaultBackground))
        expandIndicator = ta.getDrawable(R.styleable.Expansion_exp_expandIndicator)
    ...
    private fun createHeaderIcon() {
        ...
        iconImage.setImageDrawable(when (iconStyle) {
            ExpansionIconStyles.CIRCLE -> icon?.getCircleBitmap(backgroundColor)?.toDrawable(resources)
            ExpansionIconStyles.ROUNDED_SQUARE -> icon?.getRoundedSquareBitmap(backgroundColor)?.toDrawable(resources)
            else -> icon
        })
